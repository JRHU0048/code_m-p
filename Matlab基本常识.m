...
c = 100
num2str(c)        % num2str(c) 将数值变量 c 转换为字符串，并返回结果。
disp(['c的取值为' num2str(c)])   %连接并输出两个字符串
disp(strcat('c的取值为', num2str(c)))   %同上
...
%加分号与不加分号的区别在于是否在命令窗口显示结果。当我们希望执行一些操作但不想在命令窗口输出结果时，可以使用分号来抑制输出。
%而在需要查看操作结果的情况下，则可以省略分号。
